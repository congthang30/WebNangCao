        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(ProductName) ||
                    ...
                    CateId <= 0)
                {
                    ErrorMessage = "Vui lòng điền đầy đủ và hợp lệ thông tin sản phẩm.";
                    await LoadSelectListsAsync();
                    return Page();
                }
                ...
                {
                    ErrorMessage = "Trạng thái sản phẩm phải là 'hidden' hoặc 'presently'.";
                    await LoadSelectListsAsync();
                    return Page();
                }
                ...
                if (ImagePr != null && ImagePr.Length > 0)
                {
                    try
                    {
                        imagePath = await ImageHelper.SaveImageAsync(ImagePr, "products");
                    }
                    catch (Exception ex)
                    {
